...
def broadcast_message(update: Update, message_content: str, send_to_users=True, send_to_groups=True):
    ...
    for user in users:
        try:
            update.message.bot.send_message(user["chat_id"], message_content, parse_mode="Markdown")
            users_sent += 1
    ...
    for group in groups:
        try:
            update.message.bot.send_message(group["chat_id"], message_content, parse_mode="Markdown")
            groups_sent += 1
        except TelegramError as e:
